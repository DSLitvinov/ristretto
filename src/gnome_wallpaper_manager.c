...
cb_style_combo_changed (GtkComboBox *style_combo,
                        RsttoGnomeWallpaperManager *manager);
static void
update_preview_monitor (RsttoGnomeWallpaperManager *manager);
...
static gint
rstto_gnome_wallpaper_manager_configure_dialog_run (RsttoWallpaperManager *self,
                                                    RsttoFile *file,
                                                    GtkWindow *parent)
{
...
static gboolean
rstto_gnome_wallpaper_manager_set (RsttoWallpaperManager *self,
                                   RsttoFile *file)
{
    gchar *command = NULL;

    command = g_strdup_printf ("gsettings set org.gnome.desktop.background picture-uri %s",
                               rstto_file_get_uri (file));

    g_spawn_command_line_async (command, NULL);
    g_free (command);

    command = g_strdup ("gsettings set org.gnome.desktop.background picture-options scaled");

    g_spawn_command_line_async (command, NULL);
    g_free (command);

    command = g_strdup ("gsettings set org.gnome.desktop.background primary-color '#000000000000'");
...
rstto_gnome_wallpaper_manager_init (RsttoGnomeWallpaperManager *manager)
{
    ...
    gtk_combo_box_set_active (GTK_COMBO_BOX (manager->priv->style_combo), 0);

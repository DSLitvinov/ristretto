...
static void
update_preview_monitor (RsttoXfceWallpaperManager *manager);
static void
...
static gint
rstto_xfce_wallpaper_manager_configure_dialog_run (RsttoWallpaperManager *self,
                                                   RsttoFile *file,
                                                   GtkWindow *parent)
{
    ...
    rstto_wallpaper_preview_set_file (RSTTO_WALLPAPER_PREVIEW (manager->priv->preview),
...
static gboolean
rstto_xfce_wallpaper_manager_set (RsttoWallpaperManager *self,
                                  RsttoFile *file)
{
    RsttoXfceWallpaperManager *manager = RSTTO_XFCE_WALLPAPER_MANAGER (self);

    if (manager->priv->monitor == RSTTO_MONITOR_CHOOSER_ALL)
    {
        gint n_monitors = gdk_display_get_n_monitors (gdk_display_get_default ());

        for (gint i = 0; i < n_monitors; ++i)
        {
            manager->priv->monitor = i;
            rstto_xfce_wallpaper_manager_set (self, file);
        }
        manager->priv->monitor = RSTTO_MONITOR_CHOOSER_ALL;
...
rstto_xfce_wallpaper_manager_init (RsttoXfceWallpaperManager *manager)
{
    ...
    manager->priv->preview = rstto_wallpaper_preview_new ();
    manager->priv->monitor_chooser = rstto_monitor_chooser_new ();

    for (i = 0; i < n_monitors; ++i)
    {
        gdk_monitor_get_geometry (gdk_display_get_monitor (display, i), &monitor_geometry);
        rstto_monitor_chooser_add (RSTTO_MONITOR_CHOOSER (manager->priv->monitor_chooser),
                                   monitor_geometry.width, monitor_geometry.height);
    }
    manager->priv->monitor = rstto_monitor_chooser_get_selected (
        RSTTO_MONITOR_CHOOSER (manager->priv->monitor_chooser));
    update_preview_monitor (manager);

    gtk_box_pack_start (GTK_BOX (vbox), manager->priv->style_combo, FALSE, FALSE, 0);
    gtk_box_pack_start (GTK_BOX (vbox), manager->priv->preview, FALSE, FALSE, 0);
    gtk_box_pack_start (GTK_BOX (vbox), manager->priv->check_button, FALSE, FALSE, 0);
    ...
    gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (manager->priv->style_combo), _("Centered"));
    gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (manager->priv->style_combo), _("Tiled"));
    gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (manager->priv->style_combo), _("Stretched"));
    gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (manager->priv->style_combo), _("Scaled"));
    gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (manager->priv->style_combo), _("Zoomed"));

    gtk_combo_box_set_active (GTK_COMBO_BOX (manager->priv->style_combo), manager->priv->style);
    rstto_wallpaper_preview_set_style (RSTTO_WALLPAPER_PREVIEW (manager->priv->preview),
                                       (RsttoWallpaperStyle) manager->priv->style);
    ...
    gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (manager->priv->check_button),
                                  manager->priv->workspace_mode);
    g_signal_connect (manager->priv->check_button, "toggled",
                      G_CALLBACK (cb_workspace_mode_changed), manager);
...
static void
cb_workspace_mode_changed (GtkCheckButton *button,
                           RsttoXfceWallpaperManager *manager)
{
    gboolean active;
    ...
    if (manager->priv->channel != NULL)
        xfconf_channel_set_bool (manager->priv->channel, SINGLE_WORKSPACE_MODE, active);
}
...
update_preview_monitor (RsttoXfceWallpaperManager *manager)
{
    ...
    rstto_monitor_chooser_get_dimensions (RSTTO_MONITOR_CHOOSER (manager->priv->monitor_chooser),
                                          manager->priv->monitor, &width, &height);
    rstto_wallpaper_preview_set_monitor_size (RSTTO_WALLPAPER_PREVIEW (manager->priv->preview),
                                              width, height);
}
